...
void main() {
  ...
  float alpha = 1.0;
  if (uOverlayOnly > 0.5) {
    float rippleMask = smoothstep(0.004, 0.06, amount);
    color = mix(color, uTint * 1.35, clamp(amount * 2.2, 0.0, 1.0) * 0.42);
    alpha = rippleMask * 0.82;
  }
...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    gl.clearColor(0, 0, 0, overlayOnly ? 0 : 1);
    ...
    const updatePointerVisual = (point: [number, number] | null) => {
      if (!point) {
        mount.dataset.rippleHover = 'false';
        return;
      }
      mount.style.setProperty('--ripple-x', `${point[0]}px`);
      mount.style.setProperty('--ripple-y', `${height - point[1]}px`);
      mount.dataset.rippleHover = 'true';
    };
    ...
    const hideIdleRipple = () => {
      window.clearTimeout(idleTimer);
      idleTimer = window.setTimeout(() => {
        mount.dataset.rippleHover = 'false';
        waves.forEach((wave) => { wave.opacity = 0; });
      }, 500);
    };
    ...
    const loop = (now: number) => {
      ...
      if (hasVisibleWave || hadVisibleWave) {
  ...
  useEffect(() => {
    ...
    u.composite.uOverlayOnly.value = overlayOnly ? 1 : 0;
    u.composite.uHighlight.value = hexToRGB(highlightColor);
    u.composite.uTint.value = hexToRGB(tint);
  }, [rings, strength, swirl, dispersion, glint, tintAmount, grayscale, overlayOnly, highlightColor, tint]);
